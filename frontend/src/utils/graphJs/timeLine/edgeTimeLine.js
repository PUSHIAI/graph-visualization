...
export default function timeLine(id, nodeData, timeUnit) {
  ...
  this.sortTimeArr = this.timeLineUtils.sortLinksData(nodeData, this.timeUnit);
}
// 初始化时间轴
...
timeLine.prototype.initRange = function() {
  if(this.sortTimeArr.length != 0) {
    this.xAxis = {
      start: this.sortTimeArr[0],
      end: this.sortTimeArr[this.sortTimeArr.length - 1]
    };
    this.yAxis = {
      start: 0,
      end: 1
  ...
  const startTime = this.timeLineUtils.transferTime(this.xAxis.start),
        entTime = this.timeLineUtils.transferTime(this.xAxis.end);
...
timeLine.prototype.initXscale = function(xStart, xEnd) {
  ...
  xMax = this.svgWidth - this.svgPadding * 2;
  if(xMin > 0 && xMax > 0) {
    return d3
      .scaleTime()
      .domain([xStart, xEnd])
...
timeLine.prototype.initYscale = function(yStart = 0, yEnd) {
  const yMin = this.svgHeight - this.svgPadding,
  yMax = this.svgPadding * 2,
  range = [yMin, yMax];
  if(yMin > 0 && yMax > 0) {
    return d3
      .scaleLinear()
    ...
    return this.timeLineObj.svg
      ...
      .call(
        d3
          .axisRight(yScale)
          .ticks(ticks)
          .tickSize([this.svgWidth - this.svgPadding])
      )
      .call(g => {
        g.select(".domain").remove();
      })
...
timeLine.prototype.renderEntitiesByTime = function(xScale, yScale) {
  ...
  rectBoxHeight = this.svgHeight - this.svgPadding;
...
timeLine.prototype.setBrush = function() {
  const xMin = this.svgPadding * 2 - this.rectWidth,
  yMin = this.svgPadding,
  xMax = this.svgWidth - this.svgPadding * 2 + this.rectWidth,
  yMax = this.svgHeight - this.svgPadding,
  extentPosition = [
    [xMin, yMin],
    [xMax, yMax]
  ];
...
timeLine.prototype.selectRectHighLight = function() {
  // 没有刷子则返回
  if(!d3.select("rect.selection").attr("width")) return;
...
timeLine.prototype.initZoom = function(xScale, yScale) {
  ...
  xMax = this.svgWidth - this.svgPadding,
  yMax = this.svgHeight - this.svgPadding;
  if(xMin > 0 && yMin > 0 && xMax > 0 && yMax > 0 && xMax - xMin > 0 && yMax - yMin > 0) {
    const zoom = d3.zoom()
      .scaleExtent([1, 50])
      ...
      .translateExtent([
        ...
        [this.svgWidth - this.svgPadding, Infinity]
      ])
      .on("zoom", () => {
